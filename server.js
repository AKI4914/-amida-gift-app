
/**
 * 阿弥陀経なぞり × Amazonギフト自動付与システム
 * ---------------------------------------------------
 * 構成：
 *   - LIFFアプリ(public/index.html)がユーザーの「なぞり進捗」を計測
 *   - チェックポイント到達時に idToken 付きで /api/checkpoint を叩く
 *   - サーバー側で idToken を検証 → userId を確定
 *   - 抽選は先着1名限定。すでに誰かが当選していれば、以降は全員「終了」
 *   - どのルート(A/B/C)を通っても、当選者には一律 10,000円 のギフトを付与
 *       1) 未使用のギフトコードを1件ロックして取得(全体で1件のみ用意)
 *       2) LINE Messaging API のプッシュメッセージで送付
 *       3) 進捗テーブルに完了記録(以後は二重付与されない)
 *   - サーバー起動時に codes.csv があれば自動でDBに登録する(Shell不要)
 */

require('dotenv').config();
const express = require('express');
const Database = require('better-sqlite3');
const line = require('@line/bot-sdk');
const path = require('path');
const https = require('https');
const fs = require('fs');

const {
  LINE_CHANNEL_ACCESS_TOKEN,
  LINE_CHANNEL_SECRET,
  LIFF_CHANNEL_ID, // LIFFが紐づくチャネルID(idToken検証に使用)
  PORT = 3000,
} = process.env;

if (!LINE_CHANNEL_ACCESS_TOKEN || !LINE_CHANNEL_SECRET || !LIFF_CHANNEL_ID) {
  console.warn('[警告] .env の LINE_CHANNEL_ACCESS_TOKEN / LINE_CHANNEL_SECRET / LIFF_CHANNEL_ID を設定してください');
}

const lineConfig = {
  channelAccessToken: LINE_CHANNEL_ACCESS_TOKEN,
  channelSecret: LINE_CHANNEL_SECRET,
};
const lineClient = new line.Client(lineConfig);

// ---------------------------------------------------------
// DB初期化
// ---------------------------------------------------------
const db = new Database('/var/data/app.db');
db.pragma('journal_mode = WAL');

db.exec(`
CREATE TABLE IF NOT EXISTS users (
  line_user_id TEXT PRIMARY KEY,
  created_at   TEXT DEFAULT (datetime('now'))
);

CREATE TABLE IF NOT EXISTS gift_stock (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  tier        TEXT NOT NULL,           -- 'A' | 'B' | 'C'(記録用。当選賞品は一律10,000円)
  code        TEXT NOT NULL UNIQUE,    -- Amazonギフトコード
  used        INTEGER NOT NULL DEFAULT 0,
  used_by     TEXT,
  used_at     TEXT
);

CREATE TABLE IF NOT EXISTS progress (
  line_user_id TEXT NOT NULL,
  tier         TEXT NOT NULL,
  code_sent    TEXT,
  completed_at TEXT DEFAULT (datetime('now')),
  PRIMARY KEY (line_user_id, tier)
);
`);

// ---------------------------------------------------------
// 起動時に codes.csv があれば自動でギフトコードを登録する
// (Renderの無料プランではShell/One-Off Jobsが使えないため)
// codes.csv の形式(ヘッダーなし): tier,code
// ---------------------------------------------------------
try {
  const codesPath = path.join(__dirname, 'codes.csv');
  if (fs.existsSync(codesPath)) {
    const lines = fs.readFileSync(codesPath, 'utf-8').split('\n');
    const insertStock = db.prepare(
      `INSERT OR IGNORE INTO gift_stock (tier, code) VALUES (?, ?)`
    );
    let count = 0;
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      const [tier, code] = trimmed.split(',');
      if (!tier || !code) continue;
      insertStock.run(tier.trim().toUpperCase(), code.trim());
      count++;
    }
    console.log(`[起動時シード] codes.csv から ${count} 件のギフトコードを確認・登録しました`);
  } else {
    console.log('[起動時シード] codes.csv が見つかりませんでした(スキップ)');
  }
} catch (e) {
  console.error('[起動時シード] codes.csv の読み込みでエラーが発生しました:', e);
}

// tier(ルート)定義。表示ラベルのみ。金額は全ルート共通で10,000円
const TIERS = {
  A: { label: 'Amazonギフト 10,000円分', amount: 10000 },
  B: { label: 'Amazonギフト 10,000円分', amount: 10000 },
  C: { label: 'Amazonギフト 10,000円分', amount: 10000 },
};

// ---------------------------------------------------------
// Expressアプリ
// ---------------------------------------------------------
const app = express();
app.use('/public', express.static(path.join(__dirname, 'public')));

// --- LINE Webhook(友だち追加時などにuserIdを記録) ---
app.post('/webhook', line.middleware(lineConfig), (req, res) => {
  const events = req.body.events || [];
  events.forEach((ev) => {
    if (ev.source && ev.source.userId) {
      db.prepare(
        `INSERT OR IGNORE INTO users (line_user_id) VALUES (?)`
      ).run(ev.source.userId);
    }
  });
  res.sendStatus(200);
});

// checkpoint API 以降は JSON ボディを使うのでここで初めて有効化
app.use(express.json());

// --- idTokenを検証してuserIdを取得 ---
function verifyIdToken(idToken) {
  return new Promise((resolve, reject) => {
    const postData = new URLSearchParams({
      id_token: idToken,
      client_id: LIFF_CHANNEL_ID,
    }).toString();

    const req = https.request(
      {
        hostname: 'api.line.me',
        path: '/oauth2/v2.1/verify',
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Content-Length': Buffer.byteLength(postData),
        },
      },
      (res) => {
        let body = '';
        res.on('data', (c) => (body += c));
        res.on('end', () => {
          try {
            const json = JSON.parse(body);
            if (json.sub) resolve(json.sub); // sub = userId
            else reject(new Error('idToken検証失敗: ' + body));
          } catch (e) {
            reject(e);
          }
        });
      }
    );
    req.on('error', reject);
    req.write(postData);
    req.end();
  });
}

// --- ギフトコードを1件ロックして取り出す(全ルート共通・先着1名限定) ---
function grantGift(userId, tier) {
  // 1) このユーザーが、どのルートであれ、すでに当選済みならそのまま返す(冪等性の補償)
  const already = db
    .prepare(`SELECT * FROM progress WHERE line_user_id = ?`)
    .get(userId);
  if (already) {
    return { alreadyGranted: true, code: already.code_sent, tier: already.tier };
  }

  // 2) 全体(A/B/C共通)で未使用の在庫が残っているか確認。先着1名限定なので、
  //    すでに誰かに払い出し済みなら在庫切れ扱いにする
  const stock = db
    .prepare(`SELECT * FROM gift_stock WHERE used = 0 LIMIT 1`)
    .get();
  if (!stock) {
    throw new Error('在庫切れ');
  }

  db.prepare(`UPDATE gift_stock SET used = 1, used_by = ?, used_at = datetime('now') WHERE id = ?`)
    .run(userId, stock.id);

  db.prepare(
    `INSERT INTO progress (line_user_id, tier, code_sent) VALUES (?, ?, ?)`
  ).run(userId, tier, stock.code);

  return { alreadyGranted: false, code: stock.code, tier };
}

// --- チェックポイント到達API(LIFFフロントから呼ばれる) ---
app.post('/api/checkpoint', async (req, res) => {
  try {
    const { idToken, tier } = req.body;
    if (!idToken || !TIERS[tier]) {
      return res.status(400).json({ error: '無効なリクエストです' });
    }

    const userId = await verifyIdToken(idToken);
    db.prepare(`INSERT OR IGNORE INTO users (line_user_id) VALUES (?)`).run(userId);

    const result = grantGift(userId, tier);

    if (!result.alreadyGranted) {
      // LINE匿名メッセージでギフトコードを送信
      await lineClient.pushMessage(userId, {
        type: 'text',
        text:
          `🙏読誦お疲れ様でした。\n` +
          `${TIERS[tier].label}のコードをお届けします。\n\n` +
          `コード: ${result.code}\n\n` +
          `Amazonの公式サイトでチャージ残高に登録してご利用ください。`,
      });
    }

    res.json({
      ok: true,
      alreadyGranted: result.alreadyGranted,
      amount: TIERS[result.tier].amount,
    });
  } catch (e) {
    if (e.message === '在庫切れ') {
      console.error('在庫切れ:', req.body);
      return res.status(409).json({ error: '在庫切れです。今回の抽選はすでに終了しました。' });
    }
    console.error(e);
    res.status(500).json({ error: 'internal_error' });
  }
});

// --- 在庫確認用の簡易API(運営が残数表示用) ---
app.get('/api/stock', (req, res) => {
  const rows = db
    .prepare(`SELECT COUNT(*) as remaining FROM gift_stock WHERE used = 0`)
    .all();
  res.json(rows);
});

app.listen(PORT, () => {
  console.log(`Server running on http://localhost:${PORT}`);
});
